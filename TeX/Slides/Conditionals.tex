...
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{frame}[fragile]{The \bash|test| command and its friend \bash{[}}{Many tests are supported $\to$ for a complete list refer to the manual}
    \vspace{-6mm}
    \begin{center}
        \begin{minipage}{0.8\textwidth}
            \begin{description}[<only@1>][\texttt{-n STRING}]
...
\begin{frame}[fragile]{The \bash{[[} keyword}
    ...
    \begin{itemize}[<only@1>]
        ...
        \item String equality (\texttt{=}) and inequality (\texttt{!=}) comparison is changed to \alert{pattern matching} by default; use quotes to perform a string comparison
...
\begin{frame}[fragile]{Control Operators}
    ...
    \begin{description}[\texttt{\&\&}]
        \item[\texttt{\&\&}]
             ...
             \begin{lstlisting}[style=MyBash, numbers=none]
                 COMMAND_1 && COMMAND_2   # Equivalent to
