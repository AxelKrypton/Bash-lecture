    ...
    \begin{overlayarea}{\textwidth}{0.7\textheight}
        ...
        \begin{onlyenv}<4-6>
            \begin{itemize}
                ...
                \item We will learn \tc<5>{PP}{most of them}.
            ...
            \begin{onlyenv}<4>
                \begin{lstlisting}[style=MyBash, numbers=none, keywordstyle=\color{builtins-color},]
                   :         complete   export    local      shopt     unset
                   .         compgen    fc        logout     source    until
                   [         continue   fg        popd       suspend   wait
                   alias     declare    getopts   printf     test      while
                   bg        dirs       hash      pushd      times
                   bind      disown     help      pwd        trap
                   break     echo       history   read       type
                   builtin   enable     if        readonly   typeset
                   case      eval       jobs      return     ulimit
                   cd        exec       kill      set        umask
                   command   exit       let       shift      unalias
                \end{lstlisting}
            \end{onlyenv}
            \begin{onlyenv}<5>
                \begin{lstlisting}[style=MyBash, numbers=none, deleteemph={[5]:,.,[,alias, bg, break, case, cd, continue, declare, echo, eval, exec, exit, export,fg, help, if, jobs, kill, let, local,printf, pwd, read, readonly, return, set, shift, source, test, unalias, unset, until, wait, while}, emph={[7]:,.,[,alias, bg, break, case, cd, continue, declare, echo, eval, exec, exit, export,fg, help, if, jobs, kill, let, local,printf, pwd, read, readonly, return, set, shift, source, test, unalias, unset, until, wait, while}, emphstyle={[7]\color{PP}}]
                   :         complete   export    local      shopt     unset
                   .         compgen    fc        logout     source    until
                   [         continue   fg        popd       suspend   wait
                   alias     declare    getopts   printf     test      while
...
\begin{frame}[fragile]{Bash scripts}
    ...
    \begin{itemize}
        ...
        \item A new command is processed after the previous has \PP{ended} {\tiny~\{unless differently required\}}
        %\item Use your favourite text editor to compose your script
