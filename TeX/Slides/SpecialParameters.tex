...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{frame}{The Special Parameters}{\URL[PB]{https://www.gnu.org/software/bash/manual/}{Bash manual v5.2 section 3.4.2}}
    \vspace{-7mm}
    \begin{columns}
        \begin{column}{\dimexpr\paperwidth-10mm}
            \begin{description}[\texttt{1 2 \ldots}]
                ...
                \item[\texttt{*}]
                    Expands each positional parameter to a separate word\tikzmark{curlyStart}\\
    ...
    \begin{tikzpicture}[remember picture, overlay]
        \begin{scope}[scope on=<2>, PP]
            \draw[very thick, decorate, decoration={brace,amplitude=6pt}] (curlyStart -| curlyEnd) ++(5mm,1mm) -- ($(curlyEnd)+(5mm,-1mm)$)
                  node[midway, right=3mm, text width=10mm, align=center] {Exercises};
        \end{scope}
    \end{tikzpicture}
    \FrameRemark{Unquoted \bash|$*| and \bash|$@| expansions in many context are then subject to further word splitting and filename expansion.}
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
