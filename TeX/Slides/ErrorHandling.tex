...
\begin{frame}[fragile]{But what does it mean, in practice?}
    ...
    \begin{overlayarea}{\textwidth}{0.7\textheight}
        \begin{itemize}
            ...
            \item \alert<3>{Your way of programming has to adapt to all corner cases}
            \item \alert<4>{Checking and storing error codes via \PB{\texttt{\$?}} is not possible as before}
        \end{itemize}
        \vspace{2mm}
        \begin{onlyenv}<2>
        ...
        \begin{onlyenv}<3>
            \begin{lstlisting}[style=myBash]
                ...
                i=$(false)       # Simple command with command substitution
                echo "i is $i"   # This is NOT printed
            \end{lstlisting}
        \end{onlyenv}
        \begin{onlyenv}<4>
        ...
        \end{onlyenv}
        \large
        \begin{varblock}{example}[0.5\textwidth]{Spend some time on it}<5>
            This afternoon you will deal again in detail with some corner cases!
        \end{varblock}
        \begin{varblock}{}[0.65\textwidth]{And if I do not want to use it?}<5>
            \large \PB{Do I have other alternatives beyond the purist approach and enabling \texttt{errexit}?}
        \end{varblock}
        \vspace*{-1mm}
        \begin{varblock}{alert}[0.25\textwidth]{}<5>
            \PQ{Not really\ldots}
        \end{varblock}
    \end{overlayarea}
    \FrameRemark[3]{See end of section 3.7.1 of \URL[PB]{https://www.gnu.org/software/bash/manual/}{bash manual v5.0} for more information about simple commands exit code.}
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
...
\begin{frame}[fragile]{Enabling the \;\texttt{-u}\; shell option}
    ...
    \begin{lstlisting}[style=myBash, numbers=none, belowskip=-4mm]
        ...
        |+__+|
    \end{lstlisting}
    \begin{varblock}{alert}[0.95\textwidth]{Few (minor) drawbacks exist}
        \begin{enumerate}
            ...
            \item Not very recent versions of bash \URL[PP]{https://gist.github.com/dimo414/2fb052d230654cc0c25e9e41a9651ebe}{handles empty arrays differently}.
        \end{enumerate}
    \end{varblock}
