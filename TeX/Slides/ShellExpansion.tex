...
\begin{frame}[fragile]{Parameter expansion: String manipulation (IV)}{Since Bash v4.0 (2009)}
    ...
    \setbeamercolor{description item}{fg=PP}
    \begin{description}[Replace at start:]
        \item[First uppercase:] \PB{\small\texttt{\$\{parameter\^{}\}}}\\
            {\small
                Results in the expanded value of \texttt{parameter} with the first character capitalised
            }
        \item[All uppercase:] \PB{\small\texttt{\$\{parameter\^{}\^{}\}}}\\
            {\small
        ...
        \item[All lowercase:] \PB{\small\texttt{\$\{parameter,,\}}}\\
            {\small
...
\begin{frame}[fragile]{Word splitting}{\ldots{}the evil of Bash!}
    ...
    \begin{onlyenv}<1>
        ...
        \end{varblock}
        \begin{itemize}
            \item The shell treats each character of \bash|IFS| as a delimiter, and splits the results of the other expansions into words using these characters \textbf{as field terminators} (these are all dropped)
            ...
            \item \PQ{Note that if no expansion occurs, no splitting is performed}
        \end{itemize}
    \end{onlyenv}
    \begin{onlyenv}<2>
        \begin{lstlisting}[style=MyBash, style=oddnumbers, aboveskip=3mm]
    ...
    \begin{onlyenv}<3>
        \begin{lstlisting}[style=MyBash, style=oddnumbers, aboveskip=3mm, firstnumber=22]
            ...
            |+5 args: <> <home> <sciarra> <Documents> <Seminars>+|
            # Remember $'' special quoting to allow backslash
            $ aVar=$'\n   \n  xxx \t   \t  xx  \t'
            $ args ${aVar}
            |+2 args: <xxx> <xx>+|
            $ IFS=''; args ${aVar}; unset IFS
