...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{frame}{Several kind of expansions}{\URL[PB]{https://www.gnu.org/software/bash/manual/}{Bash manual v5.2 section 3.5}}
    ...
    \begin{itemize}
        ...
        \item Process substitution \tc{Gray!80}{\Remark{Done only from v5.1 when \URL[Gray]{https://www.gnu.org/software/bash/manual/html_node/Bash-POSIX-Mode.html}{Bash in POSIX Mode (item 30)}}}
    ...
    \begin{tikzpicture}[remember picture, overlay]
        \coordinate (xPos) at ($(current page.north)!0.4!(current page.north east)$);
        ...
        \node[anchor=north east, font=\scriptsize, above = 8mm of tokens] (metacharacters){
            \begin{tabular}{c}
    ...
    \begin{overlayarea}{\textwidth}{0.87\textheight}
        \begin{itemize}
            \item It comes in two forms:
                ...
                \end{enumerate}
            \item<only@1> \bash|[prefix]| and \bash|[postfix]| are optional and \textbf{might contain other brace expansions}
            ...
            \item<only@1> If \bash|X| and \bash|Y| are numbers, leading 0 are respected to make each term have the same width
...
\begin{frame}{Parameter expansion: Overview}
    ...
    \end{itemize}
    \PrepareURLsymbol[PB]
    \FrameRemark{Everything can be found in the \URL*{https://www.gnu.org/software/bash/manual/}{Bash manual v5.2 section 3.5.3}}
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
...
\begin{frame}[fragile]{Parameter expansion: String manipulation (III)}
    ...
    \begin{onlyenv}<1>
        ...
        \begin{varblock}{alert}[0.88\textwidth]{Introduced in Bash v5.2}
            If the \;\texttt{patsub\_replacement}\; shell option is enabled using \;\bash{shopt}\,, any unquoted instances of \texttt{\&} in \;\texttt{string}\; are replaced with the matching portion of pattern.
            This is intended to duplicate a common sed idiom.\\
            \alert{However, this is a breaking change: Be aware!}
        \end{varblock}
    \end{onlyenv}
    \begin{tikzpicture}[remember picture, overlay]
        \node[visible on=<1>] at ($(Opt)+(2.6,-0.4)$) {\Remark{We will discuss shell options in detail at a later point}};
    \end{tikzpicture}
    \begin{onlyenv}<2-3>
        \vspace{-4mm}
...
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{frame}[fragile]{Parameter expansion: String manipulation (V)~--~Reflection (I)}{Since Bash v4.4 (2016)}
    \setbeamerfont{description item}{series=\bfseries}
    \setbeamercolor{description item}{fg=PP}
    \begin{varblock*}{}[\textwidth]{}
        \begin{description}[Syntax:]
            \item[Syntax:] \PB{\texttt{\$\{parameter@operator\}}}\\
            {\small
    ...
    \begin{overlayarea}{\textwidth}{0.5\textheight}
        \begin{onlyenv}<1>
            ...
            \begin{description}[X:]
                ...
                \item[E]
                {\small
                    The expansion is a string that is the value of parameter with backslash escape sequences expanded as with the \texttt{\PS{\$'...'}} quoting mechanism.
                }
                \item[P]
                {\small
...
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{frame}[fragile]{Parameter expansion: String manipulation (VI)~--~Reflection (II)}{Since Bash v5.1 (2020)}
    \setbeamerfont{description item}{series=\bfseries}
    \setbeamercolor{description item}{fg=PP}
    \begin{overlayarea}{\textwidth}{0.5\textheight}
        ...
        \begin{description}[X:]
            ...
            \item<only@1>[k]
            {\small
                Like the \texttt{K} transformation, but expands the keys and values of indexed and associative arrays to separate words after word splitting. \Remark{Since Bash v5.2}
            }
        \end{description}
        \begin{onlyenv}<1>
            \begin{center}
...
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{frame}{Parameter expansion: Summary}
    \vspace{-3mm}
...
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{frame}{The Directory Stack}{\URL[PB]{https://www.gnu.org/software/bash/manual/}{Bash manual v5.2 section 6.8}}
    \vspace{-3mm}
    \begin{columns}[c]
...
\begin{frame}[fragile]{Arithmetic expansion}
    ...
    \begin{itemize}[<only@1>]
        ...
        \item Operators and their precedence are the same as in the \texttt{C} language\\ $\to\,$ \URL[PB]{https://www.gnu.org/software/bash/manual/}{Bash manual v5.2 section 6.5}
    \end{itemize}
    \begin{onlyenv}<2>
        \begin{lstlisting}[style=MyBash, style=oddnumbers, style=smaller]
            $ echo $(( 22 / 3 ))
...
\begin{frame}[fragile]{Word splitting: conclusion}
    \vspace{-2mm}
        \begin{varblock*}{example}[0.9\textwidth]{When does NOT it occur?}
        ...
        \begin{itemize}
            ...
            \item on expansions in assignments. \\[0.3em] Thus, one does not need to quote anything in a command like these:
                  ...
                  \end{lstlisting}
                  Quoting anyway will not break anything, so \PS{if in doubt, quote!}
        \end{itemize}
    \end{varblock*}
        ...
        \begin{column}{\dimexpr\paperwidth-10mm}
            ...
            \begin{description}[Command substitution:]
                ...
                \item[Command substitution:]
                    ...
                    Bash v5.3 introduced a new type as \alert{\texttt{\$\{ \ldots; \}}} or \alert{\texttt{\$\{| \ldots; \}}}\\
                    Do not use the deprecated syntax \tc{Gray!30}{\texttt{\textasciigrave\ldots\textasciigrave}}\\
                    Still to early to be discussed $\to$ \URL[PB]{https://www.gnu.org/software/bash/manual/}{Bash manual v5.2 section 3.5.4}\\[1mm]
                    {
                        \tiny
                        \alert{\texttt{\$(\ldots)}} is substituted by the standard output of the command (executed in a subshell), with any trailing newlines deleted.\\[1pt]
                        \alert{\texttt{\$\{ \ldots; \}}} is like \texttt{\$(\ldots)} but any side effects of command take effect immediately in the current execution environment.\\[1pt]
                        \alert{\texttt{\$\{| \ldots; \}}} will be discussed later.
                        \par
                    }
                \item[Process substitution:]
                    Useful, but not discussed here $\to$ \URL[PB]{https://www.gnu.org/software/bash/manual/}{Bash manual v5.2 section 3.5.6}\\
