...
\begin{frame}{Patterns in Bash}
    ...
    \begin{enumerate}
        ...
        \item Regular\tikzmark{REleft} expression {\tiny\{~Since Bash v3.0~\}}\tikzmark{RE}
    \end{enumerate}
    \begin{tikzpicture}[remember picture, overlay]
        \draw[very thick, decorate, decoration={brace,amplitude=6pt}] (curlyStart) ++(8mm,1mm) -- ($(curlyEnd -| curlyStart)+(8mm,-1mm)$) 
              node[midway, right=3mm, text width=45mm, align=center, text=PB] (FE) {Pattern matching done in Filename expansion};
        \node (RElabel) at (RE -| FE) {{\color{PT}\scriptsize\{~Cannot be used in to select filenames!~\}}};
...
\begin{frame}[fragile]{The \texttt{[\ldots]} pattern}
    ...
    \begin{itemize}
        ...
        \item Within \PB{\texttt{[\ldots]}}, character classes can be specified using the syntax \PB{\texttt{[:class:]}},\\
              ...
              \begin{lstlisting}[style=MyBash, numbers=none, xleftmargin=2mm, xrightmargin=15mm, aboveskip=3mm]
...
\begin{frame}[fragile]{Globs examples}
    \vspace{-5mm}
    \begin{onlyenv}<1>
        \begin{lstlisting}[style=MyBash, style=oddnumbers, xrightmargin=1mm]
            $ ls
    ...
    \end{onlyenv}
    \begin{onlyenv}<2>
        \begin{lstlisting}[style=MyBash, style=oddnumbers, xrightmargin=2mm, firstnumber=24]
