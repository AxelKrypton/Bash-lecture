...
\begin{frame}[fragile]{Accessing array's content (I)}{Definition, single elements and length}
    \vspace{-1mm}\centering
    \PB{\texttt{declare -p array}}\, It prints the definition of the variable \Remark{rarely needed in scripts}
    \smallskip
    \begin{lstlisting}[style=MyBash, numbers=none]
        $ declare -p array
...
\begin{frame}[fragile]{Iterating over array elements}
    ...
    \begin{itemize}
        ...
        \item It is possible to implicitly iterate over arrays in many places (e.g.\ \bash{printf}, \bash{cp})
    \end{itemize}
...
\begin{frame}[fragile]{Deleting arrays or array elements: A sneaky trap}
    ...
    \begin{lstlisting}[style=MyBash, emph={[7]a, a0}]
        ...
        file3+|
        $ declare -p a0
        |+-bash: declare: a0: not found+|  # Is it clear what happened?
    \end{lstlisting}
...
\begin{frame}[fragile]{Sparse arrays}
    ...
    \begin{tikzpicture}[remember picture, overlay]
        \node[anchor=north east] at ($(current page.north east)-(5mm,6mm)$) {\includegraphics[width=3cm, clip, trim=0 17mm 0 0]{Assumptions}};
    \end{tikzpicture}
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
    ...
    \begin{overlayarea}{\textwidth}{0.7\textheight}
        ...
        \begin{onlyenv}<2>
            \begin{lstlisting}[style=MyBash, firstnumber=10, emph={[7]associative, indexed}]
                ...
                $ declare -A associative=|+([foo]=bar [alpha]=omega [1]=one)+|
                $ echo "${indexed[${index}]}"
                |+one+|
                $ echo "${indexed[index]}"
                |+one+|
                ...
                |+__+|
                $ echo "_${associative[key + 1]}_" # no arithmetic context
                |+__+|
