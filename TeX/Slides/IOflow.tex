...
\begin{frame}<0>[label=IOflow]{The Bash script flow}
    \begin{tikzpicture}[overlay, remember picture]
        ...
        \coordinate (O) at ($(current page.center)-(0.5,0)$);
        ...
        \path[from] ($(O)+(\cubex/2,\cubey,-\cubez/2)$) edge[out=90, in=42]   node[pos=1, anchor=10] {\alert<2>{\tc<3>{Gray!50}{The \,\bash|read|\, builtin}}} ++(-2,-0.4,0)
                                                        edge[out=90, in=0]   node[pos=1, anchor=east] {\alert<2>{\tc<3>{Gray!50}{Command-line arguments}}} ++(-1.5,0.5,0)
        ...
        \node[visible on=<1-2>, font=\scriptsize, draw=PP, rounded corners] at ($(current page.south)+(0,1.2)$)
        {
            \begin{tabular}{c}
                Strictly speaking \bash|read| belongs to the discussion on file descriptors.\\
                It is simple to get the idea before discussing them, though.
            \end{tabular}
        };
    \end{tikzpicture}
    \vspace{32mm}
