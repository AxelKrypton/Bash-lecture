...
\begin{frame}{Colours and formatting in the terminal}{\URL[PB]{https://misc.flogisoft.com/bash/tip\_colors\_and\_formatting}{A good reference}}
    ...
    \begin{itemize}
        ...
        \item \alert{Prefer \bash|printf| over \bash|echo|!} \Remark{Read \URL[PB]{https://unix.stackexchange.com/a/65819/370049}{this gorgeous answer} to know more!}
    \end{itemize}
...
\begin{frame}[fragile]{Colours and format codes}
    ...
    \begin{onlyenv}<2>
        ...
        \begin{lstlisting}[style=MyBash, style=oddnumbers, xleftmargin=0mm, xrightmargin=0mm]
            ...
            |+Default+| @|\texttt{\tc{red}{\underline{Underlined}}}|@
            $ printf "XXX \e[4;91mUnderlined\e[24m still red\e[0m XXX\n"
            |+XXX+| @|\texttt{\tc{red}{\underline{Underlined} still red}}|@ |+XXX+|
...
\begin{frame}[fragile]{Cursor movements}
    \begin{onlyenv}<1>
        ...
        \begin{description}[\texttt{<Esc><L>;<C>H}xxxxx]
            ...
            \item[\texttt{<Esc>[s} \textcolor{TEXT}{or} \texttt{<Esc>7}] Save cursor position
            \item[\texttt{<Esc>[u} \textcolor{TEXT}{or} \texttt{<Esc>8}] Restore cursor position}
        \end{description}
        \PrepareURLsymbol[PB]
        \begin{varblock}{alert}[0.8\textwidth]{It's a tricky world!}
            \scriptsize
            \texttt{<Esc>7} and \texttt{<Esc>8} are historical DEC sequences, while the others are SCO/ANSI-style.\\
            Not all terminal supports both (e.g.\ \URL*{https://stackoverflow.com/a/25879581}{Terminal.app on macOS} supports DEC only).
        \end{varblock}
    \end{onlyenv}
    \begin{onlyenv}<2>
        ...
        \begin{lstlisting}[style=MyBash, numbers=none, xleftmargin=3mm, xrightmargin=3mm]
             ...
             # Here a crazy progress bar (use \r instead of saving cursor)
             $ while true; do
             >     printf '\e[s'      # or '\e7'
             >     printf '%.0s=' $(seq 1 $((RANDOM%100)))
             >     sleep 0.2
             >     printf '\e[u\e[K'  # or '\e8\e[K'
             > done
