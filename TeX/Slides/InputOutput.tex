...
\begin{frame}[fragile]{Environment variables (I)}
    ...
    \begin{itemize}
        ...
        \item Traditionally, these have names that are all capital letters, such as \bash|PATH|, \bash|SHELL|, \ldots
    ...
    \end{varblock}
    \PrepareURLsymbol[PB]
    \FrameRemark{Strictly speaking, bash variables (\URL*{https://www.gnu.org/software/bash/manual/}{Bash manual v5.2 section 5}) like \bash|BASH_VERSION| are not environment variables (e.g.\ they are not listed by \bash|env| command).}
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
...
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{frame}[fragile]{A slide about the \,\bash|read|\, builtin}
    \vspace{-2mm}
    \begin{lstlisting}[style=MyBash, numbers=none, belowskip=-7mm, style=smaller]
        $ help read
        |+read: read [-ers] [-a array] [-d delim] [-i text] [-n nchars]
                   [-N nchars] [-p prompt] [-t timeout] [-u fd] [NAME ...]+|
    \end{lstlisting}
    \begin{itemize}
        \item Reads a single line from the standard input
        \item The line is split into fields as with word splitting, and the first word is assigned to the first \texttt{NAME}, the second word to the second \texttt{NAME}, and so on, with any leftover words assigned to the last \texttt{NAME}
        \item \alert{Only the characters found in \bash|IFS| are recognized as word delimiters}
        \item By default, the backslash character escapes delimiter characters and newline
    \end{itemize}
    \begin{onlyenv}<1>
        \begin{lstlisting}[style=MyBash, aboveskip=2mm, style=oddnumbers]
            $ read aVar bVar
            |+Hello you all!+|  # <-- this is what you type
            $ echo "${aVar}|${bVar}"
            |+Hello|you all!+|
            $ read aVar bVar
            |+Hello\ you all!+| # <-- this is what you type
    ...
    \end{onlyenv}
    \begin{onlyenv}<2>
        \begin{lstlisting}[style=MyBash, aboveskip=2mm, style=oddnumbers, firstnumber=8]
            $ read -r aVar bVar # -r option disable backslash-escape
            |+Hello\ you all!+|     # <-- this is what you type
            $ echo "${aVar}|${bVar}"
            |+Hello\ you|all!+|
            $ read -p 'Give me one word: ' aVar
            |-Give me one word:-| |+Hello world+| # In black is what you type
            $ echo "${aVar}"
            |+Hello world+|                   # ALWAYS validate input!
        \end{lstlisting}
    \end{onlyenv}
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
