        ...
        \begin{itemize}[<2>]
            ...
            \item There are many instances in which a shell creates a subshell without parentheses being placed by the programmer
                  \begin{itemize}
                      ...
                      \item \PP{In command substitution \texttt{\$(\ldots)}}
...
\begin{frame}{Command substitution}
    \vspace{-1mm}
    \begin{varblock}{example}[0.9\textwidth]{The standard syntax: \texttt{\$(\ldots)}}
    ...
    \end{varblock}
    \begin{varblock}{}[\textwidth]{New in Bash v5.3}
        \begin{description}[\texttt{\$\{| command; \}}]
            \item[\texttt{\$\{ command; \}}]
                Bash executes \bash|command| in the current execution environment and captures its output, again with trailing newlines removed.\\
                Any side effects of \bash|command| take effect immediately and persist in the current environment after the command completes.
            \item[\texttt{\$\{| command; \}}]
                This construct expands to the value of the \bash|REPLY| shell variable after command executes, \textbf{without removing any trailing newlines} and \PP{the standard output of \bash|command| is not captured!}
        \end{description}
    \end{varblock}
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{frame}[fragile]{Command substitution: Examples}
    \begin{onlyenv}<1>
        \begin{lstlisting}[style=MyBash]
            $ aVar=$(echo 'hello'); echo "${aVar}"
            |+hello+|
            $ aVar=$(echo $'hello\n\n'); echo "${aVar}"
            |+hello+|
            $ aVar=$(echo $'hello\n\nworld'); echo "${aVar}"
            ...
            $ aVar=$(echo $'hello\n\nworld'); echo ${aVar}
            |+hello world+|
            $ aVar=$(echo 'hello' >&2); echo "_${aVar}_"
            |+hello+|  # Only standard output is captured!
            |+__+|
            $ aVar='AAA'; bVar=$(aVar=BBB && echo "${aVar}")
            $ echo "${aVar} ${bVar}"
            |+AAA BBB+|
            $ echo 'hello' > file.txt
            $ aVar=$(cat file.txt); echo "${aVar}"
            |+hello+|
            $ aVar=$(< file.txt); echo "${aVar}"
            |+hello+| # This should be preferred: Bash faster idiom
        \end{lstlisting}
    \end{onlyenv}
    \FrameRemark<1>{The command substitution \bash/$(cat |+file+|)/ can be replaced by the equivalent but faster \bash/$(< |+file+|)/.}
    \begin{onlyenv}<2>
        \begin{lstlisting}[style=MyBash, firstnumber=last]
            $ echo "${BASH_VERSION}"
            |+5.3.15(1)-release+|
            $ aVar='AAA'; bVar=${ aVar=BBB && echo "${aVar+BBB}"; |+}+|
            $ echo "${aVar} ${bVar}"
            |+BBB BBB+|
            $ unset -v 'aVar' 'bVar'
            $ echo "_${REPLY}_"; aVar=${| REPLY=1234 && echo 'hello'; |+}+|
            |+__+|
            |+hello+|
            $ echo "_${aVar}_ _${REPLY}_"
            |+_1234_ __+|
            # No trailing newline gets stripped!
            $ aVar=${| REPLY=$'1234\n' && echo $'hello\n'; |+}+|
            |+hello
            +|
            $ echo "_${aVar}_ _${REPLY}_"
            ...
            _ __+|
        \end{lstlisting}
        \begin{varblock}{alert}[0.88\textwidth]{Possibly too early for it}
            This is a very interesting new feature, but requires a very recent Bash version!
        \end{varblock}
    \end{onlyenv}
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
