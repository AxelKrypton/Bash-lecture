    ...
    \begin{overlayarea}{\textwidth}{0.7\textheight}
        ...
        \begin{onlyenv}<3>
            \begin{lstlisting}[style=MyBash, aboveskip=2mm, firstnumber=37]
                ...
                $ echo "2*5.3" | bc -l
                |+10.6+|
                # You should use the Here strings syntax, instead
            \end{lstlisting}
        \end{onlyenv}
    \end{overlayarea}
    \FrameRemark{Why is 82000 a super cool number? We'll come back to this in a moment when discussing Here strings syntax.}
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{frame}{Input/Output Redirection: The theory}
    ...
    \end{itemize}
    \FrameRemark{$^\star$ For \PB{here documents} and \PB{here strings} a different rule applies: Refer to the Bash manual for more details.}
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
        ...
        \begin{onlyenv}<3>
            ...
            \begin{varblock}{example}[0.92\textwidth]{How should I fix this?}
                \begin{lstlisting}[style=MyBash, numbers=none, aboveskip=2mm, belowskip=-5mm, xleftmargin=3mm, xrightmargin=3mm]
                    ...
                    $ a_command &> File        # ALSO GOOD (Bash specific)
                \end{lstlisting}
            \end{varblock}
        \end{onlyenv}
    \end{overlayarea}
...
\begin{frame}[fragile]{A kind of black hole in the operating system}
    \begin{tikzpicture}[overlay, remember picture]
        \node[anchor=north east] (fig) at ($(current page.north east)-(2.6mm,3.2mm)$) {\includegraphics[width=33mm]{BlackHole}};
    ...
    \begin{lstlisting}[style=MyBash, numbers=none]
        ...
        $ rm 'Who cares that this does not exists' 2>> /dev/null
        $ aVar=0; while read _; do (( aVar++ )); done < /dev/null
        $ echo "aVar = ${aVar}"
        |+aVar = 0+|
    \end{lstlisting}
    \medskip
...
\begin{frame}[fragile]{Here Documents (I)}
    ...
    \vspace{2mm}
    \begin{itemize}
        \item Here documents are useful to embed \textbf{short} blocks of multi-line data inside your script\\[-0.5ex]
              \Remark[0pt]{Embedding larger blocks is bad practice!}
        \item In a Here document you need to choose a \texttt{WORD} to act as a sentinel
...
\begin{frame}[fragile]{Here Strings}
    ...
    \begin{itemize}[<only@1-2>]
        \item Here strings are shorter, less intrusive and overall more convenient than Heredocs\\[-0.5ex]
    ...
    \begin{varblock}{quote}[0.5\textwidth]{Good practice}<only@1-2>
        \textnormal{Prefer Here strings to pipes whenever possible!}
    \end{varblock}
    \begin{onlyenv}<3>
    ...
    \end{onlyenv}
    \begin{onlyenv}<4-5>
        \begin{varblock}{example}[0.65\textwidth]{Why is 82000 a super cool number?}
            \uncover<5>{82000 in base 2, 3, 4 and 5 uses only 0 and 1 digits!}
        \end{varblock}
        \begin{lstlisting}[style=MyBash, aboveskip=0mm, numbers=left, aboveskip=4mm]
            $ for base in {2..5}; do
            >   printf "82000 in base %d is %s\n" \
            >       ${base} \
            >       $(bc <<< "obase=${base}; 82000")
            > done &&
            > unset base
        \end{lstlisting}
        \begin{uncoverenv}<5>
            \begin{lstlisting}[style=MyBash, aboveskip=0mm, numbers=none]
                |+82000 in base 2 is 10100000001010000
                82000 in base 3 is 11011111001
                82000 in base 4 is 110001100
                82000 in base 5 is 10111000+|
            \end{lstlisting}
        \end{uncoverenv}
    \end{onlyenv}
    \begin{tikzpicture}[remember picture, overlay, scope on=<2>]
        \path[from] (quotes) edge[out=0, in=270, looseness=0.5] node[pos=1, above, text=ALERT, draw=PQ, rounded corners=3mm, text width=30mm, align=center, font=\small] {Do not forget quotes, especially if the string contains spaces!} ++(5.2,1);
    \end{tikzpicture}
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
    ...
    \begin{onlyenv}<1-2>
        \begin{enumerate}\small
            ...
            \item This filename is passed as an argument to the current command as the result of the expansion\\[0.3em]
                  ...
                  \end{description}
        \end{enumerate}
        \begin{varblock}{}[0.82\textwidth]{The power of process substitution}<only@1>
        ...
        \begin{onlyenv}<2>
            \begin{lstlisting}[style=MyBash, numbers=none, aboveskip=3mm]
                $ ./program > >@|\tikzmark{STDOUT}|@(tee stdout.log) 2> >@|\tikzmark{STDERR}|@(tee stderr.log >&2)
            \end{lstlisting}
            \begin{tikzpicture}[remember picture, overlay,
                                every node/.style={text width=0.25\textwidth, pos=1, below, draw,
                                                   align=center, rounded corners}]
                \path[from, visible on=<2>] ($(STDOUT)-(0,1.5mm)$) edge[out=270, in=90]
                    node[PS]
                        {Standard output printed on screen and saved to \textbf{stdout.log}} ++(0mm,-8mm);
                \path[from, visible on=<2>] ($(STDERR)-(0,1.5mm)$) edge[out=270, in=90]
                    node[ALERT]
                        {Standard error printed on screen and saved to \textbf{stderr.log}} ++(0mm,-8mm);
            \end{tikzpicture}
        \end{onlyenv}
    \end{onlyenv}
    \begin{onlyenv}<3>
        \begin{lstlisting}[style=MyBash, aboveskip=0mm, xleftmargin=2mm, xrightmargin=0mm]
            ...
            \date{17 February 2025}        | \date{18 February 2025}+|
        \end{lstlisting}
    \end{onlyenv}
    \PrepareURLsymbol[PB]
    \FrameRemark<3>{$^\star$This is an incomplete statement, if taken as general rule: It is not \URL*{https://unix.stackexchange.com/questions/403783/the-process-substitution-output-is-out-of-the-order}{the full story}.}
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
