...
\begin{frame}[fragile]{Programmable completion}{A complex world plenty of opportunities: \enspace\URL[PB]{https://www.gnu.org/software/bash/manual/}{Bash manual v5.2 sections 8.6 and 8.7}}
    ...
    \begin{itemize}
        ...
        \item It is usually automatically sourced somewhere, e.g.\ in the shell configuration file.
    \end{itemize}
...
\begin{frame}[fragile]{A basic example as starting point}
    ...
    \begin{onlyenv}<5>
        \begin{lstlisting}[style=myBash, numbers=none, style=smaller]
            ...
            {
                case "$3" in      # $3 is the word preceding the word being completed
                    --inputfile )
    ...
    \begin{tikzpicture}[remember picture, overlay]
        \node[visible on=<6>, anchor=north west, text=ALERT, font=\small] (whystillbad) at ($(stillbad)+(5mm,3mm)$) {
            What is sub-optimal in this implementation?
        };
        \node[visible on=<7>, font=\small, text=PB] at (whystillbad) {
    ...
    \begin{onlyenv}<8>
        \begin{lstlisting}[style=myBash, numbers=none, aboveskip=4mm]
            ...
            # Assume 'measure' executable is findable through PATH
            $ measure <TAB>
            $ measure --<TAB-TAB>
