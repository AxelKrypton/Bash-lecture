...
\begin{frame}[fragile]{The \,\bash|while|\, and \,\bash|until|\, keywords}
    ...
    \begin{onlyenv}<3>
        \begin{lstlisting}[style=MyBash]
            # Avoid unnecessary command expansion
            #  -> You can also use SECONDS in this case!
            $ deadline=$((EPOCHSECONDS + 8)); \
            > while [[ ${delta=8} -gt 0 ]]; do
            >   echo "${delta} seconds to BOOM!"
            >   sleep 3
            >   delta=$((deadline - EPOCHSECONDS))
            > done; echo 'BOOOOOM!!'
            |+8 seconds to BOOM!
            5 seconds to BOOM!
            2 seconds to BOOM!
    ...
    \begin{onlyenv}<4>
        ...
        \end{lstlisting}
    \end{onlyenv}
    \FrameRemark<2-3>{The condition of the \;\bash|while|\; loop should rather be written using arithmetic evaluation \;\bash|((...))|\;, which we discuss tomorrow.}
    \FrameRemark<4>{In the snippet above arithmetic evaluation \;\bash|((...))|\;, which we discuss tomorrow, should be used instead of \;\bash|[[ ... ]]|\; and to increment variables.}
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
