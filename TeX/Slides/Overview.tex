...
\begin{frame}[c]{Lecture format}
    \begin{center}
        \begin{tikzpicture}[every node/.style={inner xsep=0}]
            \foreach \f [count=\n from 0] in {Calendar, Time, Placeholder, Person, Newsletter,Slides}{
                \begin{scope}
                    \clip (0,-1.36*\n) circle[radius=4.5mm];
                    \node (A\n) at (0,-1.36*\n) {\includegraphics[width=8mm]{\f}};
                \end{scope}
            }
            \node[right = of A0] {\{22..26\} June 2026};
            \node[right = of A1] (A1desc) {
                \begin{tabular}{@{}r@{\;}l}
            ...
            \draw[thick, decorate, decoration={brace,amplitude=4pt}]
                (A1desc.north east) ++(0mm,-2mm) -- ($(A1desc.south east)+(0mm,2mm)$)
                node[midway, right=3mm, font=\small] {Interactive lectures and hands-on sessions};
            \node[right = of A2] {Wonderful location in Laubach};
            \node[right = of A3] {Dr. Alessandro Sciarra};
            \node[right = of A4] {sciarra@itp.uni-frankfurt.de};
            \node[right = of A5] {\alert{Slides are full of text for later reading!} \Remark{Try to listen to me rather than reading}};
        \end{tikzpicture}
    \end{center}
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{frame}{Who am I?}
    ...
    \begin{itemize}
        ...
        \item Now more \PS{a Research Software Engineer (RSE)}
...
\begin{frame}{The mind setup of this course}
    ...
    \begin{enumerate}
        ...
        \item<3-> You will not receive \underline{all} the details (I tried to pass on the important information, though)\\
    ...
    \end{enumerate}
    \begin{tikzpicture}[remember picture, overlay]
        \node[anchor=south east, visible on=<2->, inner ysep=3mm, inner xsep=5mm]
            (fig) at (current page.south east) {\includegraphics[width=42mm]{Relax}};
    \end{tikzpicture}
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{frame}{What is \URL[PP]{https://www.gnu.org/software/bash/}{Bash}?}
    ...
    \begin{itemize}
        ...
        \item First release in 1989, current version v5.3 (July 2025) $\;\;\to\;$ \URL[PS]{http://ftp.gnu.org/gnu/bash/}{download Bash}
    \end{itemize}
...
\begin{frame}{Why should I know Bash?}%{Why not a more intuitive scripting language, e.g.\ Python?}
    ...
    \begin{overlayarea}{\textwidth}{0.5\textheight}
        ...
        \begin{varblock}{alerted}[0.64\textwidth]{}<only@2>
        ...
        \end{varblock}
        \begin{onlyenv}<3>
            \begin{varblock}{example}[0.68\textwidth]{Python can do everything Bash can do!?}
                Technically yes, just as e.g.\ C++ or Julia can do everything Python can do.
                The real question is whether you're expressing the problem at the right level of abstraction.\\
                \textbf{Bash does not compete with Python, it completes it.}
            \end{varblock}
            \begin{center}
                \includegraphics[width=0.3\textwidth]{Opportunity}
            \end{center}
        \end{onlyenv}
    \end{overlayarea}
    \PrepareURLsymbol[PB]
...
\begin{frame}{What will we explore?}
    ...
    \begin{multicols}{2}
        \begin{enumerate}
            ...
            \item \tc<2>{Wednesday}{Script autocompletion and GNU Readline}
    ...
    \begin{tikzpicture}[remember picture, overlay, every label/.append style={label distance=-2pt, font=\tiny}, scope on=<2>]
        \foreach \d/\n [count=\i] in {Monday/22, Tuesday/23, Wednesday/24, Thursday/25, Friday/26}{
...
\AddLinkToTOCfalse
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{frame}{A (first) slide about AI and LLMs}
    \begin{overlayarea}{\textwidth}{0.8\textheight}
        \begin{columns}[c]
            \hfill
            \begin{column}{0.6\textwidth}
                \centering
                \begin{varblock}{}[\textwidth]{A couple of questions?}
                    \begin{enumerate}
                        \item How often do you chat with an AI?
                        \item How often do you use an AI in agent mode?
                    \end{enumerate}
                \end{varblock}
                \begin{varblock}{alert}[0.95\textwidth]{Let's go back few years}<uncover@2>
                    In the first two days (at least), let's avoid using any AI powered tool!
                    Use your favourite search engine (not in AI-mode) and actively read sources.
                \end{varblock}
                \vspace{3mm}
                \begin{uncoverenv}<2>
            ...
            \begin{column}{0.35\textwidth}
                \vspace{5mm}
                \includegraphics[width=\columnwidth]{Ai-biased}
            \end{column}
            \hfill
        \end{columns}
    \end{overlayarea}
...
\begin{frame}{A short but intense journey}{A few more recommendations}
    ...
    \end{itemize}
    \vspace{5mm}
    {\large\PP{What do \alt<1>{you}{I} expect?}}
    \begin{itemize}[<2->]
        ...
        \item \alert{You ask \textbf{many} questions!} \Remark{«There is no such thing as a dumb question.»~--~Carl Sagan}
