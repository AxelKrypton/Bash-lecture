        ...
        \begin{tikzpicture}[every node/.style={inner xsep=0}]
            ...
            \node[right = of A1] {
                \begin{tabular}{@{}r@{\;}l}
                    ...
                    Afternoon: & 14:15 to 17:00 $\;\,${\tiny\{~only registered people~\}}
                \end{tabular}
                %Morning: 09:15 to 12:00$\quad$ Afternoon: 14:15 to 17:00
            };
            \node[right = of A2] {
                \begin{tabular}{@{}r@{\;}l}
                    Morning:   & FIAS H100 (Mon), GSC 0|07-0|08 (Tue, Wed) \\
                    Afternoon: & ITP, room 01.120 {\tiny\{~only registered people~\}}
                \end{tabular}
            };
...
\begin{frame}{References for future use}
    ...
    \begin{itemize}
        ...
        \item[] \URL[PS]{http://mywiki.wooledge.org/BashGuide}{Bash Guide}\PB{\tiny\{~Very precise guide, not completely from zero, though~\}} \tikzmark{TutorialEnd}
    ...
    \begin{tikzpicture}[remember picture, overlay]
        ...
        \path[from] (TutorialEnd) ++(-0mm,-1mm) edge[out=330, in=180] node[pos=1, right=2pt, font=\scriptsize, text=PS] {Main reference for preparing this lecture} ++(12mm, -6mm);
    \end{tikzpicture}
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{frame}{What will we explore?}
    ...
    \begin{multicols}{2}
        \begin{enumerate}
            ...
            \item \tc<2>{Monday}{Variables and special parameter}
            \item \tc<2>{Monday}{Shell expansion}
            \item \tc<2>{Monday}{Word splitting}
            \item \tc<2>{Monday}{Patterns}
            \item \tc<2>{Monday}{Conditionals and tests}
            \item \tc<2>{Tuesday}{Loops}
            \item \tc<2>{Tuesday}{Choices}
            ...
            \item \tc<2>{Wednesday}{What to do now?}
        \end{enumerate}
    \end{multicols}
    \vspace{-2mm}
    \begin{varblock}{quote}[0.5\textwidth]{}<2>
        \normalfont
...
\begin{frame}[fragile]{A slide about notation}
    ...
    \begin{varblock*}{alert}[0.57\textwidth]{Exercise colour notation}
        ...
        \end{itemize}
    \end{varblock*}
    \FrameRemark{This note usually provides further (advanced) information. Links here are worth the reading.}
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
