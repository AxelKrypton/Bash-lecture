...
\begin{frame}[c]{Lecture format}
    \begin{center}
        \begin{tikzpicture}[every node/.style={inner xsep=0}]
            ...
            \node[right = of A0] {\{25..29\} September 2023};
            \node[right = of A1] {
                \begin{tabular}{@{}r@{\;}l}
                    Morning:   & 09:15 to 12:30 \Remark{Lecture with slides}\\
                    Afternoon: & 14:15 to 17:30 \Remark{Exercise sheet -- I am here to help, use me}\\
                \end{tabular}
            };
            \node[right = of A2] {
                \begin{tabular}{@{}r@{\;}l}
...
\begin{frame}{What is \URL[PP]{https://www.gnu.org/software/bash/}{Bash}?}
    ...
    \begin{itemize}
        ...
        \item First release in 1989, current version v5.2 (Sep 2022) $\quad\to\;$ \URL[PS]{http://ftp.gnu.org/gnu/bash/}{download Bash}
    \end{itemize}
...
\begin{frame}{References for future use}
    ...
    \begin{tikzpicture}[remember picture, overlay]
        ...
        \path[to] (MeOnGitHub) ++(0,-2mm) edge[out=270, in=180] node[pos=1, anchor=west] {My GitHub page \hspace{1mm}\raisebox{-1.3mm}{\includegraphics[width=5mm]{AxelKrypton}}} ++(20mm, -4mm);
...
\begin{frame}{What will we explore?}
    ...
    \begin{tikzpicture}[remember picture, overlay, every label/.append style={label distance=-2pt, font=\tiny}, scope on=<2>]
        \foreach \d/\n [count=\i] in {Monday/25, Tuesday/26, Wednesday/27, Thursday/28, Friday/29}{
